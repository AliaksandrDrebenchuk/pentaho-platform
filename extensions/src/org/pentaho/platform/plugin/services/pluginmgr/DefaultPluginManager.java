...
public class DefaultPluginManager implements IPluginManager {
  ...
  public final boolean reload() {
    ...
    synchronized ( providedPlugins ) {
      ...
      for ( IPlatformPlugin plugin : providedPlugins ) {
        try {
          GenericApplicationContext beanFactory = getApplicationContext( plugin.getId() );
          if ( beanFactory != null ) {
            beanFactory.refresh();
          }
    ...
    for ( IContentInfo info : plugin.getContentInfos() ) {
      ...
      if ( !StringUtils.isEmpty( metaProviderClass ) ) {
        ...
        String defaultErrMsg =
            Messages.getInstance().getErrorString( "PluginManager.ERROR_0013_FAILED_TO_SET_CONTENT_TYPE_META_PROVIDER",
                metaProviderClass, info.getExtension() ); //$NON-NLS-1$
        ...
        BeanDefinition beanDef =
            BeanDefinitionBuilder.rootBeanDefinition( metaProviderClass ).setScope( BeanDefinition.SCOPE_PROTOTYPE )
                .getBeanDefinition();
        getApplicationContext( plugin.getId() ).registerBeanDefinition( METAPROVIDER_KEY_PREFIX + info.getExtension(),
            beanDef );
      }
    }
  }
  ...
  protected void initializeBeanFactory( final IPlatformPlugin plugin, final ClassLoader loader )
    ...
    if ( !( loader instanceof PluginClassLoader ) ) {
      logger
          .warn( "Can't determine plugin dir to load spring file because classloader is not of type PluginClassLoader.  "
              //$NON-NLS-1$
  ...
  protected void assertUnique( String pluginId, String beanId ) throws PlatformPluginRegistrationException {
    if ( getApplicationContext( pluginId ).containsBean( beanId ) ) {
      throw new PlatformPluginRegistrationException( Messages.getInstance().getErrorString(
    ...
    for ( String type : pws.getTypes() ) {
      ...
      BeanDefinition beanDef =
          BeanDefinitionBuilder.rootBeanDefinition( serviceClassName ).setScope( BeanDefinition.SCOPE_PROTOTYPE )
              .getBeanDefinition();
      getApplicationContext( plugin.getId() ).registerBeanDefinition( serviceClassKey, beanDef );
  ...
  // return getApplicationContext(pluginId);
  // }

  @Override
  public ListableBeanFactory getBeanFactory( String pluginId ) {
    GenericApplicationContext context = getApplicationContext( pluginId );
    return context == null ? null : context.getBeanFactory();
  }

  private GenericApplicationContext getApplicationContext( String beanId ) {
    GenericApplicationContext context = beanFactoryMap.get( beanId );
    if ( context == null ) {
      logger.error( "Not found the bean '" + beanId + "'." );
    }
    return context;
  }
    ...
    for ( IContentGeneratorInfo cgInfo : plugin.getContentGenerators() ) {
      ...
      BeanDefinition beanDef =
          BeanDefinitionBuilder.rootBeanDefinition( cgInfo.getClassname() ).setScope( BeanDefinition.SCOPE_PROTOTYPE )
              .getBeanDefinition();
      GenericApplicationContext factory = getApplicationContext( plugin.getId() );
  ...
  private Collection<String> getBeanIdsForType( String pluginId, Class<?> clazz ) {
    ...
    ListableBeanFactory fac = getApplicationContext( pluginId ).getBeanFactory();
